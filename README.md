...
**심사위원·관리자 화면 모두 네이티브**(Flutter)다. WebView 는 남아 있지 않다.
...
관리자가 오프라인을 지원하지 않는 것은 게으름이 아니라 판단이다.
**오래된 순위를 최신인 줄 알고 발표하는 사고가 연결 오류보다 훨씬 무섭다.**
관리자 토큰을 저장하지 않는 것도 같은 이유다 — 잠금 해제된 기기 하나가 행사 전체의 열쇠가 된다.

### 인쇄물은 네이티브로 그리지 않는다

최종집계표는 결재란이 있는 A4 공식 문서다. 서버의 Blade 출력이 유일한 정답이고,
Flutter 로 재현하면 미묘하게 달라질 위험만 크다. 앱은 서버에서 **10분짜리 서명 URL** 을 받아
시스템 브라우저로 넘긴다. CSV·심사위원 카드도 같은 방식이다.
...
5. **관리 화면은 서버 계산을 그대로 쓴다.** 순위·평균·선정 판정을 앱이 다시 계산하면
   웹 대시보드와 숫자가 어긋나고, 그 어긋남이 발표장에서 드러난다.
...
의존성을 올릴 때는 pub 캐시에 해당 버전이 있는지 먼저 확인할 것 — 캐시에 있으면 네트워크 없이 빌드된다.
...
9. 관리자 — 행사 선택·비밀번호 로그인, 집계가 **웹 대시보드와 같은 숫자**인지
10. 관리자 — 항목 배점 합계 100점 초과가 거절되는지, 대상·심사위원 일괄 등록이 웹과 같은 결과인지
11. 관리자 — 출력 메뉴가 **브라우저에서 열리는지**, 10분 뒤 같은 주소가 거절되는지
