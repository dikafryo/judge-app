...
class EventInfo {
  const EventInfo({required this.name, required this.isOpen, required this.isBlind});
  ...
  Map<String, dynamic> toJson() => {'name': name, 'is_open': isOpen, 'is_blind': isBlind};
}
...
class CriterionItem {
  ...
  Map<String, dynamic> toJson() =>
      {'id': id, 'name': name, 'max_score': maxScore, 'description': description};
}
...
class CandidateInfo {
  const CandidateInfo({required this.id, required this.number, this.name, this.affiliation});
  ...
  String get label => name == null || name!.isEmpty ? '$number번' : '$number. ${name!}';
  ...
  Map<String, dynamic> toJson() =>
      {'id': id, 'number': number, 'name': name, 'affiliation': affiliation};
}
...
class JudgePayload {
  ...
  bool isComplete(int candidateId) {
    ...
    return leafItems.isNotEmpty && leafItems.every((item) => given.containsKey(item.id));
  }
  ...
  factory JudgePayload.fromJson(Map<String, dynamic> json) {
    ...
    return JudgePayload(
      ...
      event: EventInfo.fromJson(json['event'] as Map<String, dynamic>? ?? const {}),
  ...
  Map<String, dynamic> toJson() => {
        ...
        'scores': {
          for (final entry in scores.entries)
            entry.key.toString(): {
              for (final score in entry.value.entries) score.key.toString(): score.value,
            },
        },
