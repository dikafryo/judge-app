...
void main() {
  testWidgets('서명란을 생략하면 기록자 이름을 요구한다', (tester) async {
    ...
    await tester.enterText(find.widgetWithText(TextField, '기록자 이름'), '김기록');
    await tester.tap(find.text('저장'));
    await tester.pumpAndSettle();
  ...
  testWidgets('행사명이 정확히 일치해야 영구 삭제를 확인할 수 있다', (tester) async {
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.widgetWithText(TextField, '행사명'), '다른 행사');
    await tester.pump();
    ...
    await tester.enterText(find.widgetWithText(TextField, '행사명'), '가을 심사');
    await tester.pump();
